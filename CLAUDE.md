...
- **Branch rulesets are free-plan-locked on private repos.** The free plan only supports branch protection / rulesets on public repos. `Invest4Talent/.github` is therefore **public** (safe: it contains only community-health files and workflow templates, no secrets). Consumer app repos that stay private cannot mark checks required for merge until the org upgrades to Team; the CI still runs on their PRs but merge blocking is discipline-based. Public consumer repos (module templates, reusable resources) get full ruleset support today.
